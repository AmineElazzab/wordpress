...
use YaySMTP\Aws3\Aws\Api\DocModel;
 ...
 */
class AwsClient implements \YaySMTP\Aws3\Aws\AwsClientInterface
{
    use AwsClientTrait;
    /** @var array */
    private $config;
    ...
    {
        return \YaySMTP\Aws3\Aws\ClientResolver::getDefaultArguments();
    }
    /**
    ...
    public function __construct(array $args)
    {
        ...
        $this->handlerList = new \YaySMTP\Aws3\Aws\HandlerList();
        $resolver = new \YaySMTP\Aws3\Aws\ClientResolver(static::getArguments());
        $config = $resolver->resolve($args, $this->handlerList);
        $this->api = $config['api'];
        $this->signatureProvider = $config['signature_provider'];
        $this->endpoint = new \YaySMTP\Aws3\GuzzleHttp\Psr7\Uri($config['endpoint']);
    ...
    public function getCommand($name, array $args = [])
    {
        ...
        return new \YaySMTP\Aws3\Aws\Command($name, $args, clone $this->getHandlerList());
    }
    ...
    private function parseClass()
    {
        ...
        if ($klass === __CLASS__) {
            return ['', 'YaySMTP\\Aws3\\Aws\\Exception\\AwsException'];
        }
        $service = substr($klass, strrpos($klass, '\\') + 1, -6);
        return [strtolower($service), "YaySMTP\\Aws3\\Aws\\{$service}\\Exception\\{$service}Exception"];
    }
    private function addSignatureMiddleware()
    {
        ...
        $resolver = static function (\YaySMTP\Aws3\Aws\CommandInterface $c) use($api, $provider, $name, $region, $version) {
            ...
            return \YaySMTP\Aws3\Aws\Signature\SignatureProvider::resolve($provider, $version, $name, $region);
        };
        $this->handlerList->appendSign(\YaySMTP\Aws3\Aws\Middleware::signer($this->credentialProvider, $resolver), 'signer');
    }
    private function addInvocationId()
    {
        // Add invocation id to each request
        $this->handlerList->prependSign(\YaySMTP\Aws3\Aws\Middleware::invocationId(), 'invocation-id');
    }
    /**
    ...
    {
        return [new \YaySMTP\Aws3\Aws\Api\Service($api, \YaySMTP\Aws3\Aws\Api\ApiProvider::defaultProvider()), new \YaySMTP\Aws3\Aws\Api\DocModel($docs)];
    }
    /**
     * @deprecated
     * @return static
