...
use YaySMTP\Aws3\Psr\Http\Message\StreamInterface as Stream;

abstract class AbstractUploader extends \YaySMTP\Aws3\Aws\Multipart\AbstractUploadManager {
   ...
   * @param array  $config
   */
  public function __construct(\YaySMTP\Aws3\Aws\AwsClientInterface $client, $source, array $config = []) {
   ...
   */
  protected function limitPartStream(\YaySMTP\Aws3\Psr\Http\Message\StreamInterface $stream) {
    // Limit what is read from the stream to the part size.
    return new \YaySMTP\Aws3\GuzzleHttp\Psr7\LimitStream($stream, $this->state->getPartSize(), $this->source->tell());
  }
  ...
  private function determineSource($source) {
    ...
    if (is_string($source)) {
      $source = \YaySMTP\Aws3\GuzzleHttp\Psr7\try_fopen($source, 'r');
    }
    // Create a source stream.
    $stream = \YaySMTP\Aws3\GuzzleHttp\Psr7\stream_for($source);
