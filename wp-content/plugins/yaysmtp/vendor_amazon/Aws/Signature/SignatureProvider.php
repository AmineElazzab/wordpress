...
use YaySMTP\Aws3\Aws\Exception\UnresolvedSignatureException;
/**
 * Signature providers.
 *
...
class SignatureProvider
{
    ...
    public static function resolve(callable $provider, $version, $service, $region)
    {
        ...
        throw new \YaySMTP\Aws3\Aws\Exception\UnresolvedSignatureException("Unable to resolve a signature for {$version}/{$service}/{$region}.\n" . "Valid signature versions include v4 and anonymous.");
    }
    /**
    ...
    public static function version()
    {
        return function ($version, $service, $region) {
            switch ($version) {
                case 's3v4':
                case 'v4':
                    return $service === 's3' ? new \YaySMTP\Aws3\Aws\Signature\S3SignatureV4($service, $region) : new \YaySMTP\Aws3\Aws\Signature\SignatureV4($service, $region);
                case 'v4-unsigned-body':
                    return new \YaySMTP\Aws3\Aws\Signature\SignatureV4($service, $region, ['unsigned-body' => 'true']);
                case 'anonymous':
                    return new \YaySMTP\Aws3\Aws\Signature\AnonymousSignature();
                default:
                    return null;
            }
        };
