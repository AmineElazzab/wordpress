...
function default_http_handler() {
  $version = (string) \YaySMTP\Aws3\GuzzleHttp\ClientInterface::VERSION;
  if ($version[0] === '5') {
    return new \YaySMTP\Aws3\Aws\Handler\GuzzleV5\GuzzleHandler();
  }
  if ($version[0] === '6') {
    return new \YaySMTP\Aws3\Aws\Handler\GuzzleV6\GuzzleHandler();
  }
 ...
 */
function serialize(\YaySMTP\Aws3\Aws\CommandInterface $command) {
  $request = null;
  $handlerList = $command->getHandlerList();
  // Return a mock result.
  $handlerList->setHandler(function (\YaySMTP\Aws3\Aws\CommandInterface $_, \YaySMTP\Aws3\Psr\Http\Message\RequestInterface $r) use (&$request) {
    $request = $r;
    return new \YaySMTP\Aws3\GuzzleHttp\Promise\FulfilledPromise(new \YaySMTP\Aws3\Aws\Result([]));
  });
