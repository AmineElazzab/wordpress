 ...
 */
class EachPromise implements \YaySMTP\Aws3\GuzzleHttp\Promise\PromisorInterface
{
    private $pending = [];
    /** @var \Iterator */
    private $iterable;
    ...
    {
        $this->mutex = false;
        $this->aggregate = new \YaySMTP\Aws3\GuzzleHttp\Promise\Promise(function () {
            reset($this->pending);
            ...
            while ($promise = current($this->pending)) {
                next($this->pending);
                $promise->wait();
                if ($this->aggregate->getState() !== \YaySMTP\Aws3\GuzzleHttp\Promise\PromiseInterface::PENDING) {
                    return;
                }
            }
        });
